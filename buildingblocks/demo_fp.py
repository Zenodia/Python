...
def pass3(data,singleton,doubleton,minsupport):
    triple={}
    old_tri={}
    singleton_keys=singleton.keys()
    #print singleton_keys[0]
    #print type(singleton_keys)
    l_single=len(singleton_keys)
    for i in range(0,l_single):
        for k,v in doubleton.iteritems():
            temp=list(chain.from_iterable([k]))
            tri=temp.append(singleton_keys[i])
            temp=list(set(temp))
            #print temp
            #print len(temp)
            if len(temp)>2:
    ...
    print "old_tri", old_tri
    # sort the triple keys
    for k in old_tri.keys():
        ktemp=sorted(list(k))
        ttemp=tuple(ktemp)
        triple[ttemp]=old_tri[k]
...
tri, freq_tri=pass3(data, d1, all_pair_count, min_support_threshold)
print " triples : tri is ", tri
print "freq triples : freq_tri is:",freq_tri
...
for items in pairs_asso.iteritems():
    print " interest for i--> j is :",items
    print get_interest(d2,all_pair_count,tri,((6,9),16),data)
